...
class PortfolioViews(Resource):
    resource_field = {
        ...
        # "core": fields.List(fields.Nested(resource_fields)),
        # "observe": fields.List(fields.Nested(resource_fields))
    }
...
class PortfolioInfoViews(Resource):
    ...
    def get(self):
        ...
        basic_info = BasicInfo.query.with_entities(
            BasicInfo.windcode, BasicInfo.sec_name, BasicInfo.fund_setupdate
        ).filter(
            BasicInfo.windcode.in_(ret), BasicInfo.type == "CSI"
        ).all()
        ...
        performance = FundPerformance.query.with_entities(
            FundPerformance.windcode, FundPerformance.indicator, FundPerformance.numeric
        ).filter(
