...
from ...models import FundManager, Classify, BasicInfo, Indicators
...
@api.resource("/manager/<string:windcode>")
class ManagerViews(Resource):
    @marshal_with(resource_fileds)
    def get(self, windcode):
        cls = Classify.query.with_entities(Classify.classify, Classify.fund_setupdate).filter(Classify.windcode == windcode).first()
        bench = BasicInfo.query.with_entities(BasicInfo.fund_benchmark).filter(BasicInfo.windcode == windcode).first()
        ...
        ret.classify = cls.classify
        ret.setup = cls.fund_setupdate
        ret.bench = bench.fund_benchmark
        ret.scale = f"{round(float(scale.numeric)/1e8,2)}亿元({scale.rpt_date.strftime('%Y-%m-%d')})"
        return ret


@api.resource("/manager/managed/")
